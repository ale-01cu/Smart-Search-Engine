from .cosine_similarity_interface import CosineSimilarityInterface
import math
...
class CosineSimilarity(CosineSimilarityInterface):
	def length(self, vector:list) -> float | int:
  	...
  	def calculate_cosine(self, query_vector:list, doc_vector:list) -> float | str:
		cosine:float = self.dot_product(query, document) / ((self.length(query) * self.length(document)) + 1)
    	return cosine